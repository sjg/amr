...
// trailing slash.
const BASE_URL = args.baseUrl || 'https://workshops.cetools.org';
...
// CASA Infrastructure
const STAGING_SERVER = args.STAGING_SERVER || process.env.DEFAULT_STAGING_SERVER;
const PROD_SERVER = args.prod_server || process.env.CODELAB_SERVER;
const SERVER = args.server || args.PROD_SERVER || args.STAGING_SERVER || process.env.CODELAB_SERVER;
...
// Publish @ CASA Infrastructure
gulp.task('publish:CASA:site', (callback) => {
  const dest_site = "/var/www/workshops.cetools.org/"
  const dest_codelabs = '/var/www/workshops.cetools.org/codelabs/'
  ...
  return gulp.src('dist/**').pipe(rsync(opts));
});

// Publish @ CASA Infrastructure
gulp.task('publish:CASA:codelabs', (callback) => {
  const dest_codelabs = '/var/www/workshops.cetools.org/codelabs/'

  const opts = {
    root: 'codelabs/',
    hostname: PROD_SERVER,
    destination: dest_codelabs,
    archive: true,
    silent: false,
    compress: true
  };

  return gulp.src('codelabs/**').pipe(rsync(opts));
});

gulp.task('publish:CASA:workshops', (callback) => {
  const dest_codelabs = '/var/www/workshops.cetools.org/codelabs/'
